...
If you're unsure whether to write a node or a modifier, the rule of thumb is: *is this effect always part of this camera's behavior, or is it conditional?* Always-present → node. Conditional → modifier.
...
Each of the four recipes below is self-contained — you can jump directly to the relevant one:
...
- [Custom Transitions](custom-transitions.md) — the smallest recipe; pose-only contract, four-phase lifecycle, velocity handling, testing in isolation.
- [Custom Modifiers](custom-modifiers.md) — blueprint-authored in most cases, but C++ modifiers make sense for performance-sensitive or cross-project reusable effects.
