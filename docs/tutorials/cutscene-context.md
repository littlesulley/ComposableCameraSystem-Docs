...
2. The trigger fires `PushCameraContext("Cutscene")`, which makes `Cutscene` the top of the stack and activates a cinematic camera in it.
...
4. A timer fires `PopCameraContext("Cutscene")` (or `TerminateCurrentCamera`), which triggers an [inter-context transition](../user-guide/concepts/context-stack.md#inter-context-transitions-why-the-old-context-keeps-evaluating) back to gameplay.
...
3. Drop a **Push Camera Context** node from the palette. Wire:
    - `World Context Object` = `Self`
    - `Player Index` = `0`
    - `Context Name` = `Cutscene`
    - `Camera Type Asset` = `CT_StatueOrbit`
    - `Focus Actor` = the statue reference. (This pin appears because of the context parameter you declared on the type asset.)

That's the push. The PCM:

- Calls `EnsureContext("Cutscene")` — adds or moves-to-top the `Cutscene` context.
...
For this tutorial, just fire the pop on a timer three seconds in. In the same trigger Blueprint, right after the `PushCameraContext` call:

1. Drop a **Delay** node. `Duration` = `3.0`.
2. Drop a **Pop Camera Context** node. `Context Name` = `Cutscene`.
...
- **Cutscene snaps in, no blend** — `CT_StatueOrbit.EnterTransition` is null, and nothing else supplied a transition. Either set the enter transition as in step 3, or pass a caller override via the `Transition Override` pin on `Push Camera Context`.
