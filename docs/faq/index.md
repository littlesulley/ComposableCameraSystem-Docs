...
UE 5.6 or newer. The plugin uses APIs that don't exist in 5.5 and earlier, and no backport is planned. The current source has also been adjusted for UE 5.7/Fab compatibility.
...
### A Fab or unity build points at `Module.ComposableCameraSystem.*.cpp`.

`Module.*.cpp` files are generated by UnrealBuildTool for unity builds. Do not edit those files directly. If the error mentions duplicate helper names from anonymous namespaces, update to the latest plugin source first; current builds use file-specific helper names for the known Fab collisions in runtime debug, Sequencer track, screen-space debug, spawn-track, and viewport-transform tooling.

For project-specific plugin changes, keep anonymous-namespace helper functions and variables file-specific instead of reusing generic names such as `ResolvePCM`, `ResolveLSComponent`, `DrawCanvasRect`, or `StartupCallbackHandle` across many `.cpp` files. Fab can compile several source files into one generated unity translation unit, so generic internal names can collide even though non-unity local builds pass.
