...
# UComposableCameraCubicTransition { #ucomposablecameracubictransition }

```cpp
#include <ComposableCameraCubicTransition.h>
```
...
|--------|------|-------------|
| `FComposableCameraPose` | [`OnEvaluate_Implementation`](#onevaluate_implementation-2) `virtual` |  |
| `float` | [`GetBlendWeightAt`](#getblendweightat-2) `virtual` `const` | Evaluate the transition's timing curve at a given normalized progress in [0, 1]. Returns the blend weight this transition would apply at that progress — i.e. the shape of its Percentage-over-duration curve. |
| `void` | [`DrawTransitionDebug`](#drawtransitiondebug-2) `virtual` `const` | Per-transition world-space debug hook. |

---

#### OnEvaluate_Implementation { #onevaluate_implementation-2 }
...
#### GetBlendWeightAt { #getblendweightat-2 }
...
#### DrawTransitionDebug { #drawtransitiondebug-2 }
