...
| `float` | [`GetBlendWeightAt`](#getblendweightat-3) `virtual` `const` | Evaluate the transition's timing curve at a given normalized progress in [0, 1]. Returns the blend weight this transition would apply at that progress — i.e. the shape of its Percentage-over-duration curve. |
| `void` | [`DrawTransitionDebug`](#drawtransitiondebug-4) `virtual` `const` | Per-transition world-space debug hook. |
...
#### GetBlendWeightAt { #getblendweightat-3 }
...
#### DrawTransitionDebug { #drawtransitiondebug-4 }
