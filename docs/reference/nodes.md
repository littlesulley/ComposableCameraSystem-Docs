...
Camera node that wraps the Composition Solver — produces a complete camera pose (Position + Rotation + FOV + Focus + Aperture) from an authored `FComposableCameraShot` each tick. This is the consumer that connects the Shot data model to the camera evaluation pipeline.

The Shot is authored fully in this node's Details panel inside the camera type asset. The node has **no pins** — it owns the camera pose entirely. Pose-overwriting: Position, Rotation, FieldOfView, Aperture, and FocusDistance are unconditionally written when the solver succeeds. `PhysicalCameraBlendWeight` is forced to 1.0. **Patch compatibility:** `Incompatible`.
