...
!!! tip "More debug tools are available"
    `showdebug camera` is one of four debug surfaces in CCS. The newer tools offer richer real-time views and clipboard dumps without leaving PIE:
...
- [Debug Panel & Dump Commands](debug-panel.md) — the richer CCS debug surfaces: `CCS.Debug.Panel`, `CCS.Debug.Viewport`, `CCS.Dump.*`
- [User Guide → Debugging](../../user-guide/debugging.md) — workflow guide covering all four debug surfaces and when to use each
