...
## Shot Zone Overlay (`FComposableCameraShotZoneOverlay`)

!!! note "Auto-drafted — please review"
    This section was generated by the docs auto-updater from the class header. Expand with screenshots and workflow notes when available.

A 2D screen-space framing-zone overlay introduced in V2.2 that mirrors the Shot Editor's framing-zone preview in PIE, Game, and Sequencer-scrub viewports. For every active `UComposableCameraCompositionFramingNode` whose `Aim.AimZones` or `Placement.PlacementZones` has `bEnabled == true`, the overlay draws:

- translucent dead-zone and soft-zone rectangles centered on the configured `ScreenPosition`,
- the resolved anchor disc (equivalent to the Shot Editor's §D.4.1 preview), and
- the projection marker.

### Activation

```
CCS.Debug.Viewport.ShotZones 1   ← enable framing-zone overlay
CCS.Debug.Viewport.ShotZones 0   ← disable (default)
```

This CVar is **independent** of the 3D-gizmo master switch (`CCS.Debug.Viewport`). The two groups address different diagnostic needs — 3D world-space gizmos vs. 2D screen-space framing zones — and do not entangle.

### Rendering pathway

Uses `UDebugDrawService::Register("Game" / "Editor", ...)` with the same dual-channel + per-(frame, `FCanvas*`) dedup pattern as `FComposableCameraDebugPanel`, so the overlay survives the F8 eject viewport swap and doesn't double-draw when one viewport transiently has both `ShowFlags` set.

The source of truth for "which Shots are currently active" is `UComposableCameraCompositionFramingNode::GetActiveInstances()`. Both the PCM gameplay path and the LS Component path register to this registry (both run the same node), so the overlay shows all active framing Shots regardless of how the camera is being driven.

### Build gating

All overlay cost is `#if !UE_BUILD_SHIPPING`. `Initialize()` / `Shutdown()` are no-ops in Shipping. Module-owned lifecycle: `FComposableCameraSystemModule::StartupModule` calls `Initialize`; `ShutdownModule` calls `Shutdown`.
